...
def load_data(wavpath,of='raw', denoiser=False, **extra):
    ...
    if of == 'raw':    
        ...
        subprocess.call(['ffmpeg','-y', '-i', wavpath,'-ar', '16000', '-ac', '1', '-hide_banner', '-loglevel', 'error', output_wavpath])
        if denoiser:
            ...
            subprocess.call(['ffmpeg','-y', '-i', output_wavpath_enhanced,'-ar', '16000', '-ac', '1', '-sample_fmt', 's16', '-hide_banner', '-loglevel', 'error', output_wavpath])


        #os.remove(wavpath)
        #wavpath = wavpath+'_new.wav'
        print(f"Loading wav file from {output_wavpath}")
        wav= pydub.AudioSegment.from_file(output_wavpath, sample_width=2, frame_rate=16000, channels=1)
    elif of == 'url': 
