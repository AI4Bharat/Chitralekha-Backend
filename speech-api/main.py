...
for k,m in config.items():
    if m['model_type'] == 'IndicWav2Vec':
        ...
        beam_search = nemo_asr.modules.BeamSearchDecoderWithLM(
                        vocab=vocab,
                        beam_width=128,
                        alpha=0.7, beta=-0.5, #TODO: Change the values
                        lm_path=m['lm_path'],
...
def align_nemo(fp_arr, DEVICE, lang, restore_punct=True):
    ...
    # model.decoder.freeze()
    with torch.no_grad():
        logits, logits_len, greedy_predictions = asr.forward(input_signal=feature.unsqueeze(0).cuda(), input_signal_length=lengths)
...
def download_drive_audio(url):
    ...
    subprocess.call(['ffmpeg', '-y', '-i',downloaded_audio_path,'-ar', '16k', '-ac', '1', '-hide_banner', '-loglevel', 'error', downloaded_audio_path+'new.wav'])
...
async def transcribe_audio(audio_request: AudioRequest):
    ...
    return process_audio(audio_url, vad_val, chunk_size, language, is_denoiser, retsore_punct)
...
def process_audio(audio_url, vad_val, chunk_size, language, denoiser=False, restore_punct=True):
    status = "SUCCESS"
    ...
    if language == 'en':
        ...
        for idx, segment in enumerate(result["segments"]):
            op += str(idx+1) + '\n'
            op += "{0}.000 --> {1}.000".format(time.strftime('%H:%M:%S', time.gmtime(segment["start"])),time.strftime('%H:%M:%S', time.gmtime(segment["end"])))+'\n'
            op+= segment["text"]
            op += '\n\n'
