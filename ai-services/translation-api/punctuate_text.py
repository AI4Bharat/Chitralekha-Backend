...
class Punctuation:
    def __init__(self, language_code):
        ...
        if self.language_code in ["en", "en_bio"]:
            ...
            self.model_path = (
                cache + "deployed_models/model_data/punctuation_en_distilbert.nemo"
            )
            ...
            self.model_path = (
                cache + "deployed_models/model_data/" + self.language_code + ".pt"
            )
            self.albert_metadata = cache + "deployed_models/model_data/albert_metadata/"
            self.encoder_path = (
                cache + "deployed_models/model_data/" + self.language_code + ".json"
            )
            self.dict_map = (
                cache
                + "deployed_models/model_data/"
                + self.language_code
                + "_dict.json"
            )
            (
    ...
    def download_model_data(self):
        ...
        if not os.path.exists(cache + "deployed_models/model_data/albert_metadata"):
            os.makedirs(
                cache + "deployed_models/model_data/albert_metadata/", exist_ok=True
            )

        if self.language_code in ["en", "en_bio"]:
            if (
                len(os.listdir(cache + "deployed_models/model_data/transformers_cache"))
                != 15
            ):
                wget.download(
                    f"https://storage.googleapis.com/vakyaansh-open-models/punctuation_models/en/distilbert_base_uncased_huggingface_files.zip",
                    cache + "deployed_models/model_data/",
                    bar=self.bar_thermometer,
                )
                shutil.unpack_archive(
                    cache
                    + "deployed_models/model_data"
                    + "/distilbert_base_uncased_huggingface_files.zip",
                    cache + "deployed_models/model_data/transformers_cache/",
                )
    ...
    def punctuate_text_others_sentence(self, sentence):
        ...
        for i in range(1, len(tokens) - 1):
            if tokens[i].startswith("▁"):
                ...
                new_labels.append(
                    list(self.train_encoder.keys())[
                        list(self.train_encoder.values()).index(label_indices[0][i])
                    ]
                )
        ...
        new_labels = [
            "blank" if x == "PAD" else x for x in new_labels
        ]  # fix for PAD predicted in outputs
    ...
    def punctuate_text_others_buffer(self, sentence, buffer_length=50):
        ...
        while sentence_length > buffer_length:
            ...
            end_position = max(
                [full_stop_position, viram_position, question_mark_position]
            )
            beg_word = len(
                txt[:end_position]
    ...
    def punctuate_english_sentence(self, sentence, buffer_length=400):
        ...
        while sentence_length > buffer_length:
            ...
            txt = (
                txt
                + self.model.add_punctuation_capitalization([sentence_segment])[0]
                + " "
            )
            full_stop_position = len(txt) - txt[::-1].find(".") - 1
            question_mark_position = len(txt) - txt[::-1].find("?") - 1
            end_position = (
                full_stop_position
                if full_stop_position >= question_mark_position
                else question_mark_position
            )
            beg_word = len(
                txt[:end_position]
                .translate(str.maketrans("", "", string.punctuation + "।"))
