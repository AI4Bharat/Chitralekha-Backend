...
class OrganizationViewSet(viewsets.ModelViewSet):
    ...
    @action(
        detail=True,
        methods=["POST"],
        name="Remove Org members",
        url_name="remove_org_members",
    )
    @is_particular_organization_owner
    def remove_org_members(self, request, pk=None, *args, **kwargs):
        try:
            organization = Organization.objects.get(pk=pk)
        except Organization.DoesNotExist:
            return Response(
        ...
        try:
            ...
            if user.organization != organization and request.is_admin == False:
                return Response(
                    {
                        "message": "This user can't be deleted as this user belongs to another organization."
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
            if user.role == "ORG_OWNER":
                return Response(
                    {
                        "message": "This user can't be deleted as Organization Owners can't be deleted."
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
            if request.user == user:
                return Response(
                    {"message": "This user can't be deleted as can't delete self."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        except User.DoesNotExist:
            return Response(
        ...
        projects = Project.get(organization_id=organization)
        all_assigned_tasks = []
